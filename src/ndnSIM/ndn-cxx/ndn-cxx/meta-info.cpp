...
//******************Added by Yuhang
MetaInfo&
MetaInfo::setNextHopRtt(uint32_t thisHopRtt)
{
  m_wire.reset();
  m_nextHopRtt = thisHopRtt;
  return *this;
}

MetaInfo&
MetaInfo::setPathId(uint32_t pathId)
{
  m_wire.reset();
  m_pathId = pathId;
  return *this;
...
MetaInfo::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  // NextHopRtt added by Yuhang
  if (m_nextHopRtt != 0) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::NextHopRtt, m_nextHopRtt);
  }
  // PathId added by Yuhang
  if (m_pathId != 0) {
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::PathId, m_pathId);
  }
...
MetaInfo::wireDecode(const Block& wire)
{
  ...
  // PathId
  if (val != m_wire.elements_end() && val->type() == tlv::PathId) {
    m_pathId = readNonNegativeIntegerAs<uint64_t>(*val);
    ++val;
  }
  else {
    m_pathId = 0;
  ...
  // NextHopRtt
  if (val != m_wire.elements_end() && val->type() == tlv::NextHopRtt) {
    m_nextHopRtt = readNonNegativeIntegerAs<uint64_t>(*val);
    ++val;
  }
  else {
    m_nextHopRtt = 0;

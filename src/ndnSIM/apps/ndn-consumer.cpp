...
Consumer::SendPacket()
{
  ...
  std::cout << "> Interest for " << seq << std::endl;
...
Consumer::OnData(shared_ptr<const Data> data)
{
  ...
  dataPtr->wireDecode(dataPtr->m_wire);
  uint64_t rtt = data->getNextHopRtt();

  // std::cout << "rtt is " << rtt << std::endl;
  // std::cout << "Content Size is " << dataPtr->getContent().size() << std::endl;
  // std::cout << "PathId: " << dataPtr->getPathId() << std::endl;
...
{
  //~ std::cout << "Timeout Sequence: " << sequenceNumber << std::endl;

...
Forwarder::onOutgoingInterest(const shared_ptr<pit::Entry>& pitEntry, Face& outFace, const Interest& interest)
{
  ...
  this->dispatchToStrategy(*pitEntry,
    [&] (fw::Strategy& strategy) { strategy.afterSendInterest(pitEntry, outFace, interest); });
...
Forwarder::onInterestFinalize(const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  // trigger strategy: after Interest Finalize, By Yuhang Ye
  this->dispatchToStrategy(*pitEntry,
    [&] (fw::Strategy& strategy) { strategy.afterInterestFinalize(pitEntry); });
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  // Get the send out time

  shared_ptr<Data> dataCopyWithoutTag = make_shared<Data>(data);
  dataCopyWithoutTag->removeTag<lp::HopCountTag>();

  // Reset the Field: By Yuhang Ye <It the field is implemented using a Tag, then no need to modify>
  dataCopyWithoutTag->wireDecode(dataCopyWithoutTag->m_wire);
  dataCopyWithoutTag->setContentType(0); // 0: Payload
  dataCopyWithoutTag->setNextHopRtt(0);  // 0: Local
  dataCopyWithoutTag->setPathId(0);  // 0: Local
  dataCopyWithoutTag->wireEncode();
  // ****************
  ...
  // and send Data to all matched out faces
    ...
    for (const shared_ptr<pit::Entry>& pitEntry : pitMatches) {
      ...
      for (const pit::InRecord& inRecord : pitEntry->getInRecords()) {
        if (inRecord.getExpiry() > now) {
          pendingDownstreams.insert(std::pair<Face*, uint64_t>(&inRecord.getFace(), 1234));
        }
      }
    ...
    for (std::pair<Face*, uint64_t> pendingDownstream : pendingDownstreams) {
      if (pendingDownstream.first->getId() == inFace.getId() &&
      ...
      dataPtr->setNextHopRtt(pendingDownstream.second);
      dataPtr->wireEncode();
      
      // goto outgoing Data pipeline
      this->onOutgoingData(data, *(pendingDownstream.first));
    }
  }
}
